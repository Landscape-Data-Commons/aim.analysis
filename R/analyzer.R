...
#' @param evaluated.points Data frame output from benchmarker()
#' @param weights Data frame output from weighter(), equivalent to weighter()[["point.weights"]] or weighter()[[2]]. Optionally, the output from weight.adjuster() instead
#' @param tdat Data frame or SpatialPointsDataFrame containing the TerrADat PrimaryKey field and reporting units for the points found in evaluated.points
#' @param adjustedweights Logical, defaults to TRUE. If the field ADJWGT is found in the data frame provided for the argument weights, should it be used instead of the field WGT? Allows for quick comparison between the results of analysis with and without the weight adjustment
#' @return A data frame with the
#' @keywords analysis
#' @examples
#' analyzer()

analyzer <- function(evaluated.points,
                     weights,
