...
apply_benchmarks <- function(data,
                             ...
                             verbose = FALSE,
                             tdat_version = "2") {
  if (!any(c("sf", "data.frame" %in% class(data)))) {
  ...
  if (!is.null(benchmark_group_lut)) {
    ...
    data_grouped <- dplyr::left_join(x = data,
  ...
  data_benchmarked <- dplyr::left_join(x = data_tall,
                            y = benchmarks,
                            by = c("indicator" = "indicator_var",
  ...
  output <- dplyr::select(.data = data_benchmarked,
                          tidyselect::all_of(idvars),
                          indicator,
                          Condition.Category)[benchmark_vector]
