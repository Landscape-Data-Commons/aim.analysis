...
2. The **data summary** provides contextual information that is helpful for interpreting the results, including the list of indicators, timing of data collection, and maps.
...
The following tabs contain tables and graphs describing the information in this report and the data collection effort. This includes the a map of the monitoring sites and reporting units, the indicators being considered, and the timing of data collection.

## Map of Study Area, Reporting Units, and Sample Points
The map below is provided for general reference only. It was created from the spatial data stored in the project's Sample Design Database, the reporting units provided for this analysis, and the actual monitoring site locations recorded in TerrADat.
...
for (val in c("YES", "NO", "UNCERTAIN")) {
  d <- reporting.unit.summary.df[grepl(paste(val,"$",sep=""),
                           toupper(reporting.unit.summary.df$Objective.Met)),]
  d <- d[, c("Monitoring.Objective", "Indicator", "Objective.Met", "reporting.unit")]
  ...
  for (reporting.unit in reporting.units) {
    ...
    for (mq in mqs) { # by specific reporting unit
      ...
      for (indicator in Indicators) {  # by indicator
        ...
        map <- indicatorMap(reporting.unit.level = type,
                         reporting.unit.name = reporting.unit,
