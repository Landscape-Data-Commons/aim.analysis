...
#'
#' This function takes the output from the function sdd.reader() and optionally a SpatialPolygonsDataFrame defining the extent of reporting units. Returns a list of two data frames: strata.weights with the weighting information by stratum and point.weights with the weighting information by point.
#' @param sdd.import Output from sdd.reader(). This can be the result of running sdd.reader() on one or many sample design databases
#' @param combine Logical. If provided multiple SDDs, should those be combined as part of the weighting calculations? Otherwise the weights will be calculated on a per-SDD basis. Defaults to TRUE.
#' @param reorder Logical. If provided multiple SDDs, should those be reordered to reflect ascending area? If FALSE, the SDDs are considered in the order they appear in sdd.import. If combine is FALSE, then this only potentially affects the order of the output information. Defaults to T.
...
## TODO: Figure out what to do about SDDs that have future points
...
weighter <- function(sdd.import, ## The output from sdd.reader()
  ...
  for (s in sdd.order) {
    ...
    if (!is.null(sdd.import$strata[[s]])) {
      ...
      stratum.summary <- eval(parse(text = paste0("pts.summary.wide %>% group_by(SDD,", "WEIGHT.ID", ", YEAR) %>% dplyr::summarize(sum(", paste0(extant.counts, collapse = "), sum("), "))")))
      ...
      ## N.B. I removed the references to the project area because that should be determined by now and not relevant, but you can add this if you need to
      # "TERRA_PRJCT_AREA_ID",
      stratum.summary <- stratum.summary %>% group_by_("SDD", "WEIGHT.ID") %>%
      ...
      stratum.summary <- stratum.summary[, c("SDD",
                                             "Stratum",
                                             "Total.pts",
                                             "Observed.pts",
      ...
      if ("UNIQUE.IDENTIFIER" %in% names(frame.spdf@data)) {
        pointsweights.current <- weight.adjuster(points = pointsweights.current,
