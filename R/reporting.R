...
#' A wrapper for \code{plot_fate_distribution()} to apply that splits the input \code{point.weights} by the variable REPORTING.UNIT and returns a list
...
#' @export
mulitplot_fate_distribution <- function(point.weights,
                                        fates = NULL){
  plot.list <- lapply(X = split(point.weights, "REPORTING.UNIT"),
         FUN = plot_fate_distribution,
         fates = fates)
