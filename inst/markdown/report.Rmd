...
```{r plotDates, echo=FALSE, message=FALSE, warning=FALSE}
  ...
  ggplot2::ggplot(data = dplyr::distinct(dplyr::select(.data = points.benchmarked, PRIMARYKEY, yday, year)),
...
for (level in reporting.unit.levels) { # by reporting unit level
  ...
  for (reporting.unit in unique(data.reporting.unit.level$Subpopulation)) {
    prop.table.reporting.unit <- addLSProp(prop.table = prop.table,
                                           analysis.table = analysis,
                                           reporting.unit = reporting.unit,
                                           indicator.lut = indicator.lut)
    prop.table.reporting.unit <-  prop.table.reporting.unit[,-c(2, 5, 6, 8)] %>% dplyr::arrange(MANAGEMENT.QUESTION, EVALUATION.CATEGORY)  # sort by mgt question and category
    prop.table.reporting.unit <- prop.table.reporting.unit %>% dplyr::select(-StdError.P)
    names(prop.table.reporting.unit) <- c("Goal/Management Objective",
                              "Indicator",
                              "Category",
                              "Required Percent",
                              "n",
                              ...
                              "Upper CI",
                              "Objective Met?")
    prop.table.reporting.unit.strip <- prop.table.reporting.unit[!(prop.table.reporting.unit$Category %in% cats.to.suppress),] # Strip out the categories to suppress
    
    ## Dump these into a list so we can print them out later. Need to first generate the summary table
    tables.hold[[reporting.unit]] <- xtable::xtable(prop.table.reporting.unit.strip,
                                       align = c("l","l","l","l","c","c","c","c","c","c"))
    
    prop.table.reporting.unit$level <- level
    prop.table.reporting.unit$reporting.unit <- reporting.unit
    summary.table <- rbind(summary.table,
                           as.data.frame(prop.table.reporting.unit))
  }
}
...
for (level in reporting.unit.levels) {
  ...
  for (reporting.unit in unique(data.reporting.unit.level$Subpopulation)) {
    cat("\n### ",reporting.unit,"\n")
    print(tables.hold[[reporting.unit]],
          type = 'html',
          include.rownames = FALSE,
          html.table.attributes = list("border='0' cellpadding='5' class='bordered_table' "))
  }
