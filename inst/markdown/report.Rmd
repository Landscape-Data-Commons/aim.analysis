...
Information about the health of the landscapes we manage is essential to achieve the BLM mission (Federal Land Policy and Management Act of 1976).  Land health information enables BLM to measure the benefits that these lands and waters provide, understand the effects of multiple uses, and thus create management plans to sustain these landscapes for future generations.

Achieving management goals and objectives on BLM lands requires a structured implementation process whereby management goals are refined into concrete monitoring objectives. These monitoring objectives identify the indicator of interest, acceptable range of values for this indicator (i.e., the benchmark), the desired percentage of the project area achieving this objective, the desired confidence level and the time period over which this indicator will be evaluated.  With monitoring data are evaluated to determine whether the monitoring objective is achieved and then the user can decide if a management goal is also achieved.
...
3. The **goals, monitoring objectives, benchmarks, and results summary** section is where results begin. This contains tables of which indicators met specified objectives by reporting unit, and a summary of the stated objectives.  These table can be used by BLM land managers to evaluate whether goals or management objectives were achieved. These tables summarize results for all indicators.
...
# Table of strata summarization
print(xtable::xtable(strata.sampling.table),
      type = 'html',
